import * as React from "react"
import { cva, type VariantProps } from "class-variance-authority"
import { cn } from "@/lib/utils"

const badgeVariants = cva(
  "inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
        default: "border-transparent bg-primary text-primary-foreground",
        secondary: "border-transparent bg-secondary text-secondary-foreground",
        destructive: "border-transparent bg-destructive text-destructive-foreground",
        outline: "text-foreground",
        denied: "border-transparent bg-status-denied/10 text-status-denied",
        rejected: "border-transparent bg-status-rejected/10 text-status-rejected",
        pending: "border-transparent bg-status-pending/10 text-status-pending",
        underpaid: "border-transparent bg-status-underpaid/10 text-status-underpaid",
        resolved: "border-transparent bg-status-resolved/10 text-status-resolved",
        appealed: "border-transparent bg-status-appealed/10 text-status-appealed",
      },
    },
    defaultVariants: {
      variant: "default",
    },
  }
)

export interface BadgeProps
  extends React.HTMLAttributes<HTMLDivElement>,
    VariantProps<typeof badgeVariants> {}

function Badge({ className, variant, ...props }: BadgeProps) {
  return (
    <div className={cn(badgeVariants({ variant }), className)} {...props} />
  )
}

export { Badge, badgeVariants }
